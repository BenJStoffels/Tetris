    ...
    def inner(s):
        ...
        for y, l in enumerate(player["shape"]):
            ...
            for x, e in enumerate(l):
                if e > 0:
                    pygame.draw.rect(s, COLORS[e] if not white else (255, 255, 255), ((
...
def drawBoard(s, b, player=None, quit_action=None):
    ...
    if player:
        # we zetten de huidige blok in print_b, zonder het huidige veld aan te passen
        player.burn(print_b)

    pygame.draw.rect(s, COLORS[0], (0, 0, window_width, window_height))
    for y, l in enumerate(print_b):
        pygame.draw.rect(
            s, COLORS[8], (0, y * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))
        for x, e in enumerate(l):
            if e > 0:
                pygame.draw.rect(
                    s, COLORS[e], ((x + 1) * SQUARE_SIZE, y * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))

        pygame.draw.rect(s, COLORS[8], ((
            WIDTH + 1) * SQUARE_SIZE, y * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))

    pygame.draw.rect(s, COLORS[8], (0, window_height -
                                    SQUARE_SIZE, (WIDTH + 2) * SQUARE_SIZE, SQUARE_SIZE))
    ...
    pygame.draw.rect(
        s, COLORS[8], (min_x + 6 * SQUARE_SIZE, min_y, SQUARE_SIZE, 7 * SQUARE_SIZE))
    pygame.draw.rect(s, COLORS[8], (min_x, min_y +
                                    6 * SQUARE_SIZE, 7 * SQUARE_SIZE, SQUARE_SIZE))
...
def drawMenu(s, prev_score, current_level, play_button_action=None, level_select_action=None):
    """ Fuck off """
    pygame.draw.rect(s, COLORS[0], (0, 0, window_width, window_height))
    displayText(s, "Tetris", (window_width / 2,
                              window_height / 10), center=True)

    if prev_score:
        displayText(s, prev_score, (window_width / 2,
                                    window_height / 2 + 50), center=True)

    displayButton(s, "PLAY!", (window_width / 2, window_height *
                               0.80, 120, 50), (255, 255, 255), (0, 0, 0), play_button_action)

    displayButton(s, "00",
                  (window_width / 2 - 50, 0.2 * window_height, 50, 50),
                  (252, 209, 42) if (current_level // 10) *
                  10 == 0 else (255, 255, 255),
                  (0, 0, 0), level_select_action, 0)
    displayButton(s, "10",
                  (window_width / 2, 0.2 * window_height, 50, 50),
                  (252, 209, 42) if (current_level // 10) *
                  10 == 10 else (255, 255, 255),
                  (0, 0, 0), level_select_action, 10)
    displayButton(s, "20",
                  (window_width / 2 + 50, 0.2 * window_height, 50, 50),
                  (252, 209, 42) if (current_level // 10) *
                  10 == 20 else (255, 255, 255),
                  (0, 0, 0), level_select_action, 20)

    for i in range(10):
        displayButton(s, str(i),
                      (window_width / 2 + ((i % 5) - 2) * 50,
                       0.2 * window_height + 52 + (i // 5) * 50,
                       50, 50),
